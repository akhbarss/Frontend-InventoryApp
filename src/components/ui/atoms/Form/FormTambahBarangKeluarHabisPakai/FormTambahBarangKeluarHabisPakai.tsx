...
type FormTambahBarangKeluarHabisPakaiProps = {
  categoryItem: CategoryItem.BARANG_HABIS_PAKAI;
};

export const FormTambahBarangKeluarHabisPakai = React.memo(
  ({ categoryItem }: FormTambahBarangKeluarHabisPakaiProps) => {
    console.log("form");
    const { user } = useAuth();
    ...
    const dataItems: ComboboxData = useMemo(
      () =>
        datas.map((item) => ({
          label: `${item.name} ${item?.item_code ? item.item_code : ""}`,
          value: item.id + "",
        ...
        return (
          <Group key={item.key} mt={"xs"} w={"100%"}>
            ...
            <TextInput
              required
              style={{ flex: 1 }}
              {...form.getInputProps(`item_details.${index}.jumlah`)}
            />
            <ActionIcon
              color="red"
              size={"lg"}
